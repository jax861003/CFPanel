// 云端部署器 —— edgetunnel 单源版（托管版，Cloudflare Pages Functions）
// 登录：Cloudflare 邮箱 + Global API Key
// 部署目标：https://github.com/cmliu/edgetunnel 的 _worker.js
...
// 构建 edgetunnel 的 plain_text 变量绑定（ADMIN 必填，其余为空则忽略）
function 构建变量(数据) {
  const 变量 = [];
  const 添加 = (名称, 值) => {
...
function 生成保留Worker元数据(设置) {
  ...
  if (!Array.isArray(元数据.bindings)) throw new Error('无法读取现有 Worker 绑定，已停止更新以避免覆盖变量/KV 配置');
  return 元数据;
}
...
async function 读取源代码(context) {
  const 远程地址 = `${部署源.repoBase}/${encodeURIComponent(部署源.fileName)}?t=${Date.now()}`;
